-- Phase 2c extension: add 'teacher_principal' as a direct_conversations relationship_type.
-- This lets teachers message the principal directly (and vice versa) using the same
-- direct_conversations/direct_messages tables that power parent_teacher and parent_principal.
...
alter table public.direct_conversations
  ...
  check (relationship_type in ('parent_teacher', 'parent_principal', 'teacher_principal', 'principal_principal', 'peer_help'));

-- Update get_or_create_direct_conversation to validate teacher_principal relationships.
...
begin
  ...
  if p_relationship_type = 'parent_teacher' then
    ...
    if not (
      (v_caller_role = 'teacher' and v_other_role = 'owner')
      or (v_caller_role = 'owner' and v_other_role = 'teacher')
      or (v_caller_role = 'teacher' and v_other_role = 'admin')
      or (v_caller_role = 'admin' and v_other_role = 'teacher')
    ) then
      raise exception 'A teacher-principal conversation requires a teacher and the principal/admin (owner)';
    end if;
