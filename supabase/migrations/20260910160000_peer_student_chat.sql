-- Allow students to use direct conversations for peer help.
-- Both participants must be active students in the same organization; sections may differ.
...
begin
  ...
  if p_relationship_type = 'peer_help' then
    if p_context_type not in ('school', 'college') or p_organization_id is null then
      raise exception 'A peer-help conversation requires a school or college organization';
    end if;
    ...
    if p_context_type = 'school' then
      ...
      where organization_id = p_organization_id
        ...
        and status = 'active';
      select member_role into v_other_role
      from public.school_memberships
      where organization_id = p_organization_id
        and profile_id = p_other_profile_id
        and status = 'active';
    else
      select member_role into v_caller_role
    ...
    if v_caller_role is distinct from 'student' or v_other_role is distinct from 'student' then
      raise exception 'A peer-help conversation requires two active students in this organization';
    end if;

  elsif p_relationship_type = 'parent_teacher' then
    if p_context_type not in ('school', 'college') or p_organization_id is null then
  ...
  elsif p_relationship_type = 'teacher_principal' then
    if p_context_type not in ('school', 'college') or p_organization_id is null then
      raise exception 'A teacher-principal conversation requires a school or college organization';
    end if;
    if p_context_type = 'school' then
      select member_role into v_caller_role from public.school_memberships
        where organization_id = p_organization_id and profile_id = v_caller and status = 'active';
    ...
    if not (
      (v_caller_role = 'teacher' and v_other_role in ('owner', 'admin'))
      or (v_caller_role in ('owner', 'admin') and v_other_role = 'teacher')
    ) then
      raise exception 'A teacher-principal conversation requires a teacher and the principal/admin';
    end if;
  ...
  insert into public.direct_conversations (
    context_type, organization_id, relationship_type, participant_one_id, participant_two_id
  )
