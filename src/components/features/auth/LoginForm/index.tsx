  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Apne account mein login karo</p>
      </div>
      <OAuthButtons />
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="relative">
          ...
          <Input {...register('password')} type={showPass ? 'text' : 'password'} placeholder="Password" className="pl-10 pr-10" error={errors.password?.message} />
          <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
            {showPass ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
          </button>
        </div>
        <div className="flex justify-end">
          <Link href="/forgot-password" className="text-sm text-primary hover:underline">Password bhool gaye?</Link>
        </div>
      ...
      <p className="text-center text-sm text-muted-foreground mt-6">
        Account nahi hai? <Link href="/register" className="text-primary font-medium hover:underline">Register karo</Link>
      </p>
    </div>
  );
}
