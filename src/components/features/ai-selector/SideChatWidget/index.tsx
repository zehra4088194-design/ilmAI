...
const SEARCH_STOP_WORDS = new Set([
  'about','answer','batao','chahiye','from','hai','hain','help','kahan','kaise','karo','kro','kya','mein','mujhe','please','question','show','this','where',
]);
...
const HIDE_ON_ROUTES = [
  '/mcq','/diagnostic','/practice','/full-test','/guess-paper','/student-chat','/parent','/settings','/ai-tutor','/scan','/notes',
  '/university/pdf-summarizer','/university/project-builder','/university/assignment-helper','/university/essay-assistant','/university/presentation-builder','/university/pharmapulse',
];

function getTeacherLinks(pathname: string | null) {
  if (pathname?.startsWith('/school')) {
    return [
      { label: 'Teacher AI Tools', href: '/school/teacher-tools' },
      { label: 'Photo → Test', href: '/school/photo-test' },
    ];
  }
  if (pathname?.startsWith('/college')) {
    return [
      { label: 'Teacher AI Tools', href: '/college/teacher-tools' },
      { label: 'Photo → Test', href: '/college/photo-test' },
    ];
  }
...
export function SideChatWidget() {
  ...
  const persistMessages = async (nextMessages: SideChatMessage[], explicitId?: string) => {
    if (!user?.id || !chatLoaded) return;
    const supabase = createClient();
    const id = explicitId || conversationId || crypto.randomUUID();
    const firstUser = nextMessages.find((message) => message.role === 'user');
    const payload = {
      id,
      user_id: user.id,
      title: firstUser?.content?.slice(0, 80) || 'Quick Help',
      subject_id: null,
    ...
    if (!error) setConversationId(id);
    else console.warn('Side chat persistence failed:', error.message);
  };
  ...
  const handleSend = async () => {
      ...
      void getDynamicSearchLinks(text).then((dynamicLinks) => {
        ...
        setMessages((current) => {
          const updated = current.map((message) => {
            ...
            return { ...message, links: Array.from(new Map(merged.map((link) => [link.href, link])).values()).slice(0, 8) };
          });
    ...
    try {
      const res = await fetch('/api/ai/chat', {
        ...
        body: JSON.stringify({ message: text, history: messages.slice(-6), provider, tier, source: 'side_chat', pageLabel: derivePageLabel(pathname) }),
      });
      if (!res.ok) {
      ...
      const finalMessages = [...messages, userMsg, { ...assistantMsg, content: full }];
      await persistMessages(finalMessages);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Something went wrong.';
      const finalMessages = [...messages, userMsg, { ...assistantMsg, content: errorMessage }];
  ...
  return (
    <>
      <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} onClick={() => setIsOpen(!isOpen)} className={cn('fixed right-5 bottom-5 z-[70] flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-violet-600 to-indigo-600 shadow-2xl shadow-violet-500/30', isOpen && 'hidden')} aria-label="Open quick help chat">
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            {isExpanded && (
              <motion.button
                aria-label="Close expanded chat backdrop"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                onClick={() => setIsExpanded(false)}
                className="fixed inset-0 z-[79] cursor-default border-0 bg-black/25 p-0 backdrop-blur-md"
              />
            )}
            <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.95 }} className={dialogClass}>
              <div className="border-border/80 flex shrink-0 items-center gap-2 border-b bg-background/70 p-3 backdrop-blur-xl">
                ...
                <Button type="button" size="icon-sm" variant="ghost" onClick={() => setIsExpanded((value) => !value)} aria-label={isExpanded ? 'Minimize chat' : 'Expand chat'} title={isExpanded ? 'Minimize' : 'Expand'}>
              ...
              <div className="flex-1 space-y-3 overflow-y-auto bg-background/55 p-3 backdrop-blur-sm">
                ...
                {messages.map((message) => (
                  <motion.div key={message.id} initial={{ opacity: 0, y: 10, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.18, ease: 'easeOut' }} className={cn('flex', message.role === 'user' && 'justify-end')}>
                    <div className={cn('max-w-[85%] rounded-xl px-3 py-2 text-sm shadow-sm', message.role === 'user' ? 'bg-primary text-primary-foreground' : 'border-border/70 bg-card/90 text-card-foreground border')}>
