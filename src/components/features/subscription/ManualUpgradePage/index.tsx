  ...
  async function beginCheckout() {
    ...
    try {
      const response = await fetch('/api/payments/create-session', {
        ...
        body: JSON.stringify({ tier, billingCycle: billing, provider: 'paddle', planFamily }),
      });
      ...
      <Card className="overflow-hidden border-violet-500/30">
        <div className={`bg-gradient-to-r ${highlight} p-6 text-white`}>
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-white/15"><Icon className="h-6 w-6" /></div>
            <div>
              <Badge className="mb-2 bg-white/20 text-white hover:bg-white/20">{isOneTime ? 'One-time pass' : billing === 'annual' ? 'Yearly' : 'Monthly'} plan</Badge>
              <h1 className="text-3xl font-bold">Go {plan.name}</h1>
              <p className="mt-1 text-white/85">{CURRENCY_SYMBOLS[displayCurrency]}{formatPrice(price, displayCurrency)} {isOneTime ? 'one-time · 30 days' : billing === 'annual' ? '/year' : '/month'}<span className="ml-2 text-sm text-white/70">{feeDisplay}</span></p>
            </div>
          </div>
        </div>
        <CardContent className="space-y-5 p-6">
          <div className="grid gap-3 sm:grid-cols-2">{benefits.map((benefit) => <div key={benefit} className="bg-muted/20 flex items-center gap-2 rounded-xl border p-3 text-sm font-medium"><CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-500" />{benefit}</div>)}</div>
          {!paymentAvailability.consumptionOnly && !hasActiveSubscription && (
            <div className="bg-muted/20 grid gap-4 rounded-2xl border p-5 sm:grid-cols-2">
              <label className="space-y-2 text-sm font-semibold"><span>Billing country</span><select value={country} onChange={(event) => { setCountry(event.target.value as CheckoutCountry); setMethod('card'); }} className="bg-background h-11 w-full rounded-xl border px-3 font-normal"><option value="PK">Pakistan</option><option value="OTHER">Other country</option></select></label>
              <div className="space-y-2"><p className="text-sm font-semibold">Payment method</p><div className="grid grid-cols-2 gap-2 sm:grid-cols-3"><button type="button" onClick={() => setMethod('card')} className={`rounded-xl border px-3 py-2 text-sm font-medium ${method === 'card' ? 'border-primary bg-primary/10 text-primary' : 'bg-background'}`}>Visa / Mastercard</button>{country === 'PK' && <button type="button" onClick={() => setMethod('wallet')} className={`rounded-xl border px-3 py-2 text-sm font-medium ${method === 'wallet' ? 'border-primary bg-primary/10 text-primary' : 'bg-background'}`}>JazzCash / Easypaisa</button>}</div></div>
            </div>
          )}
          ...
          : hasActiveSubscription ? <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-5"><h2 className="text-lg font-bold">Paid plan already active</h2><p className="text-muted-foreground mt-2 text-sm leading-6">New checkout is blocked to prevent duplicate subscriptions and double charges. Contact support to change your plan or billing cycle.</p></div>
          : method === 'card' ? <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-5"><h2 className="text-lg font-bold">{isOneTime ? 'One-time Visa / Mastercard payment' : 'Visa / Mastercard subscription'}</h2><p className="text-muted-foreground mt-2 text-sm leading-6">{isOneTime ? 'Paddle charges once. Your PRO/ELITE entitlement remains active for 30 days and does not renew automatically.' : 'Paddle handles recurring USD billing, 3D Secure, receipts, and automatic renewal.'}</p><Button type="button" variant="gradient" className="mt-4 w-full sm:w-auto" disabled={!paymentAvailability.paddleConfigured} loading={checkoutLoading} onClick={beginCheckout}><CreditCard className="h-4 w-4" /> Pay ${formatPrice(usdPrice, 'USD')} by card</Button><p className="text-muted-foreground mt-2 text-xs">+${TRANSACTION_FEE_USD.toFixed(2)} transaction fee applies on top of the price shown.</p>{!paymentAvailability.paddleConfigured && <p className="text-muted-foreground mt-3 text-xs">Checkout will become active after Paddle keys and matching product IDs are configured.</p>}</div> : null}
          {!paymentAvailability.consumptionOnly && !hasActiveSubscription && country === 'PK' && method === 'wallet' && (
            <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-5"><h2 className="text-lg font-bold">JazzCash / Easypaisa</h2><p className="text-muted-foreground mt-2 text-sm leading-6">Send exactly Rs. {formatPrice(walletTotalPkr, 'PKR')} (plan + transaction fee), then submit the payment proof. An admin verifies it before activating access.</p><div className="mt-4 space-y-3">{MANUAL_PAYMENT_OPTIONS.map((option) => <div key={option.label} className="bg-background flex flex-col items-center gap-4 rounded-xl border p-4 text-center sm:flex-row sm:text-left">{option.hasQr && <div className="rounded-lg bg-white p-4">{walletQrDataUrl ? <img src={walletQrDataUrl} alt={`${option.label} payment QR`} width={132} height={132} /> : <div className="text-muted-foreground flex h-[132px] w-[132px] items-center justify-center text-center text-xs">QR unavailable</div>}</div>}<div className="min-w-0 flex-1"><p className="text-xs font-semibold uppercase tracking-wide text-amber-600">{option.label}</p><p className="text-lg font-bold">Rs. {formatPrice(walletTotalPkr, 'PKR')}</p><p className="text-muted-foreground text-sm">{option.accountName}</p><button type="button" onClick={() => copy(option.number)} className="mt-2 inline-flex items-center gap-2 rounded-lg border px-3 py-1.5 text-sm font-semibold transition-colors hover:border-amber-500/50">{option.number}<Copy className="text-muted-foreground h-3.5 w-3.5" /></button></div></div>)}</div><div className="mt-4"><PaymentProofForm context={`${plan.name} ${isOneTime ? 'one-time' : billing} plan payment confirmation`} /></div><p className="text-muted-foreground mt-2 text-xs">Never share an OTP or wallet PIN.</p></div>
          )}
        </CardContent>
      </Card>
    </div>
