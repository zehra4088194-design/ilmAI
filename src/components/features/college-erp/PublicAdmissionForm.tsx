...
export function PublicAdmissionForm({
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const recaptchaToken = await getRecaptchaToken('college_admission');
    ...
    <form onSubmit={submit} className="border-border bg-card rounded-lg border p-4 shadow-sm sm:p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="space-y-1.5 text-sm font-medium">
          ...
          <select name="applying_for_program" required className={selectClass}>
        ...
        <label className="space-y-1.5 text-sm font-medium">
          ...
          <Input name="previous_institution" maxLength={160} />
        </label>
