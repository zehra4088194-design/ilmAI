...
// College mirror of src/components/features/school-erp/SchoolImportWizard.tsx.
...
import { importCollegeStaff, importCollegeStudents } from '@/lib/college-erp/import-actions';
import { INITIAL_COLLEGE_IMPORT_STATE, type CollegeImportState } from '@/lib/college-erp/import-types';

const selectClass = 'border-input bg-background h-10 w-full rounded-lg border px-3 text-sm';

const STUDENT_TEMPLATE = toCsv(
  ['full_name', 'registration_number', 'roll_number', 'email', 'phone', 'section', 'guardian_name', 'guardian_email', 'guardian_phone', 'guardian_relationship'],
  [
    ['Ayesha Khan', '2026-C-001', '1', '', '03001234567', 'BS-CS-1A', 'Imran Khan', '', '03007654321', 'father'],
    ['Bilal Ahmed', '2026-C-002', '2', 'bilal@example.com', '', 'BS-CS-1A', 'Sara Ahmed', 'sara@example.com', '', 'mother'],
  ]
);
...
const STAFF_TEMPLATE = toCsv(
  ...
  [
    ['Hina Raza', 'hina@example.com', 'teacher', 'Assistant Professor', 'EMP-01', '03001112233'],
...
function ImportResult({ state }: { state: CollegeImportState }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <textarea
        ...
        rows={6}
        spellCheck={false}
        placeholder="full_name,registration_number,..."
...
export function CollegeImportWizard({
  years,
  sections,
}: {
  ...
  const [studentState, studentAction, studentPending] = useActionState(
    ...
    INITIAL_COLLEGE_IMPORT_STATE
  );
  const [staffState, staffAction, staffPending] = useActionState(importCollegeStaff, INITIAL_COLLEGE_IMPORT_STATE);
        ...
        <CardContent>
          <form action={studentAction} className="space-y-3">
            ...
            <p className="text-muted-foreground text-[11px]">
              Required: <code>full_name</code>, <code>registration_number</code>. Students without an{' '}
              <code>email</code> get a login generated from their registration number. Guardian columns create the
              parent account and link it automatically.
            </p>
