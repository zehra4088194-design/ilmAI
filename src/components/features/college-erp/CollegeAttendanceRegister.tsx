...
import { saveCollegeAttendance } from '@/lib/college-erp/actions';
import { INITIAL_COLLEGE_ACTION_STATE } from '@/lib/college-erp/types';

// Mirrors src/components/features/school-erp/AttendanceRegister.tsx, pointed at the college action.
const STATUSES = [
  { value: 'present', label: 'P', title: 'Present', active: 'border-emerald-500 bg-emerald-500/15 text-emerald-700 dark:text-emerald-300' },
  { value: 'absent', label: 'A', title: 'Absent', active: 'border-rose-500 bg-rose-500/15 text-rose-700 dark:text-rose-300' },
  { value: 'late', label: 'L', title: 'Late', active: 'border-amber-500 bg-amber-500/15 text-amber-700 dark:text-amber-300' },
  { value: 'excused', label: 'E', title: 'Excused', active: 'border-sky-500 bg-sky-500/15 text-sky-700 dark:text-sky-300' },
] as const;
...
type AttendanceRecord = { section_id: string; student_id: string; status: string; remarks: string | null };

export function CollegeAttendanceRegister({
  sections,
  enrollments,
  records,
  date,
  ...
  const entries = students.map((student) => ({ studentId: student.student_id, status: answers[student.student_id] || 'present' }));
    ...
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="space-y-1 text-xs font-medium">
          Section
          <select value={sectionId} onChange={(event) => setSectionId(event.target.value)} className="border-input bg-background h-10 w-full rounded-lg border px-3 text-sm">
            {sections.map((section) => (
              <option key={section.id} value={section.id}>{section.college_semesters?.name || 'Semester'} - {section.name}</option>
            ))}
          </select>
        </label>
        <label className="space-y-1 text-xs font-medium">
          Date
          <input type="date" value={attendanceDate} onChange={(event) => setAttendanceDate(event.target.value)} className="border-input bg-background h-10 w-full rounded-lg border px-3 text-sm" />
        </label>
      </div>
      ...
      <div className="border-border overflow-hidden rounded-lg border">
        ...
        <div className="divide-border divide-y">
          {visibleStudents.map((student) => {
            ...
            return (
              <div key={student.student_id} className="grid min-h-14 grid-cols-[minmax(150px,1fr)_150px] items-center gap-2 px-3 py-2">
          ...
          {!students.length && <p className="text-muted-foreground p-6 text-center text-sm">No active students in this section.</p>}
          {students.length > 0 && !visibleStudents.length && <p className="text-muted-foreground p-6 text-center text-sm">No students match &quot;{query}&quot;.</p>}
        </div>
      </div>
