...
export function UniversityDashboard({ profile }: Props) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <section className="overflow-hidden rounded-2xl border bg-gradient-to-br from-violet-500/15 via-background to-cyan-500/10 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {UNIVERSITY_TOOLS.map((tool) => (
          <Link key={tool.href} href={tool.href} className="group h-full">
            <Card className="h-full border-border/70 bg-card/90 transition-all hover:-translate-y-1 hover:border-violet-500/40 hover:shadow-lg">
          ...
          <CardContent className="p-5">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {courses.length > 0 ? courses.map((course) => (
                <span key={course} className="rounded-full border bg-muted/30 px-3 py-1 text-sm">{course}</span>
              )) : <p className="text-sm text-muted-foreground">Courses Settings me add karo taake prompts aur plans zyada personal ho jayen.</p>}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-5">
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              Use AI output as a study draft. Review, personalize, verify facts, and add real references before submission.
            </p>
            <p className="mt-3 text-xs text-muted-foreground">Preferred style: {profile?.preferred_output_style || 'simple'}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {UNIVERSITY_FEATURES.map((feature) => (
          <div key={feature.label} className="flex items-center gap-3 rounded-xl border bg-card/80 p-4 text-sm font-medium">
