...
export function SchoolActionForm({ action, children, submitLabel, className = 'space-y-3' }: { action: SchoolServerAction; children: ReactNode; submitLabel: string; className?: string }) {
  ...
  return (
    <form action={formAction} encType="multipart/form-data" className={className}>
      {children}
      {state.message && (
        <p role="status" className={`flex items-center gap-2 text-xs ${state.success ? 'text-emerald-600 dark:text-emerald-400' : 'text-destructive'}`}>
