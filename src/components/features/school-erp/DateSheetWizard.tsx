...
import { createExamScheduleBatch } from '@/lib/school-erp/actions';

type Section = { id: string; name: string; school_classes?: { name: string } | { name: string }[] | null };
...
function sectionLabel(section: Section) {
  ...
  return `${klass?.name ? `${klass.name} - ` : ''}${section.name}`;
}

/**
 * Guided, question-by-question date-sheet builder (Part 4.1): asks one exam slot at a time rather
 * than presenting a freeform table the principal has to fill in unaided. "Add another exam?" loops
 * back to the first question until the principal is done, then a single bulk save
 * (createExamScheduleBatch) writes the whole compiled date sheet.
 */
export function DateSheetWizard({ exams, sections }: { exams: Exam[]; sections: Section[] }) {
  ...
  const canAdvance = (() => {
    switch (question) {
      case 'section':
        return Boolean(draft.sectionId);
      case 'subject':
        return draft.subjectName.trim().length > 0;
      case 'date':
        return /^\d{4}-\d{2}-\d{2}$/.test(draft.examDate);
      case 'time':
  ...
  const saveDateSheet = async () => {
    ...
    try {
      ...
      const state = await createExamScheduleBatch({ success: false, message: '' }, form);
          ...
          {entries.map((entry, index) => (
            <div key={`${entry.sectionId}-${entry.subjectName}-${index}`} className="flex items-center justify-between gap-2 text-xs">
              <span className="truncate">
                {sections.find((s) => s.id === entry.sectionId) ? sectionLabel(sections.find((s) => s.id === entry.sectionId)!) : 'Section'} —{' '}
                {entry.subjectName} — {entry.examDate}
              </span>
              {stage === 'review' && (
      ...
      {stage === 'asking' && (
        <div className="border-border space-y-3 rounded-lg border p-4">
          <Badge variant="outline">
            Exam slot {entries.length + 1} — question {questionIndex + 1} of {QUESTIONS.length}
          </Badge>
          {question === 'section' && (
            <label className="block space-y-1 text-sm font-medium">
              Which class / section is this exam for?
              <select
          ...
          {question === 'time' && (
            <div className="grid grid-cols-2 gap-3">
              <label className="block space-y-1 text-sm font-medium">
                Start time (optional)
                <Input type="time" value={draft.startsAt} onChange={(event) => setDraft({ ...draft, startsAt: event.target.value })} />
              </label>
              <label className="block space-y-1 text-sm font-medium">
                End time (optional)
                <Input type="time" value={draft.endsAt} onChange={(event) => setDraft({ ...draft, endsAt: event.target.value })} />
              </label>
            </div>
          )}
          {question === 'room' && (
            <label className="block space-y-1 text-sm font-medium">
              ...
              <Input
                ...
                placeholder="Room 12 — Mr. Ahmed"
              />
            </label>
          )}
          <div className="flex items-center justify-between pt-2">
            <Button type="button" variant="ghost" size="sm" onClick={back} disabled={questionIndex === 0}>
      ...
      {stage === 'confirm-another' && (
        <div className="border-border flex flex-wrap items-center justify-between gap-3 rounded-lg border p-4">
          ...
          <div className="flex gap-2">
            <Button type="button" size="sm" onClick={() => addAnother(true)}>
              <Plus className="h-3.5 w-3.5" />
              Yes, add another
            </Button>
            <Button type="button" size="sm" variant="outline" onClick={() => addAnother(false)}>
              No, I&apos;m done
            </Button>
          </div>
        </div>
          ...
          <Button type="button" onClick={saveDateSheet} disabled={saving || !entries.length}>
            {saving ? 'Saving...' : (
              <>
                ...
                Save date sheet ({entries.length} exam{entries.length === 1 ? '' : 's'})
              </>
            )}
          </Button>
          <Button type="button" variant="outline" onClick={() => setStage('confirm-another')}>
