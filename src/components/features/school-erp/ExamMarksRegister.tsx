...
import { saveExamMarks } from '@/lib/school-erp/actions';
import { INITIAL_SCHOOL_ACTION_STATE } from '@/lib/school-erp/types';

export function ExamMarksRegister({
  schedules,
  enrollments,
  marks,
  canManage,
    ...
    () =>
      Object.fromEntries(
        marks
          ...
          .map((mark) => [
            mark.student_id,
            { marks: mark.marks_obtained == null ? '' : String(mark.marks_obtained), absent: mark.is_absent },
          ])
      ),
    [marks, scheduleId]
  );
  const [values, setValues] = useState<Record<string, { marks: string; absent: boolean }>>(existing);
  const [state, action, pending] = useActionState(saveExamMarks, INITIAL_SCHOOL_ACTION_STATE);
  const entries = students.map((student) => {
    const value = values[student.student_id] || existing[student.student_id] || { marks: '', absent: false };
    return {
      studentId: student.student_id,
      marks: value.marks === '' ? 0 : Number(value.marks),
      absent: value.absent,
    };
  });
    ...
    <div className="space-y-4">
        ...
        <select
          ...
          onChange={(event) => {
            ...
            const nextExisting = Object.fromEntries(
              marks
                .filter((mark) => mark.schedule_id === event.target.value)
                .map((mark) => [
                  mark.student_id,
                  { marks: mark.marks_obtained == null ? '' : String(mark.marks_obtained), absent: mark.is_absent },
                ])
        ...
        >
          {schedules.map((item) => (
            <option key={item.id} value={item.id}>
              {item.school_exams?.name || 'Exam'} -{' '}
              {item.school_classes?.name || item.school_sections?.school_classes?.name || 'Class'}{' '}
              {item.school_sections?.name} - {item.subject_name}
            </option>
          ))}
        </select>
      </label>
      ...
      <div className="border-border overflow-hidden rounded-lg border">
        ...
        <div className="divide-border divide-y">
          {students.map((student) => {
            ...
            return (
              <div
                ...
                className="grid min-h-14 grid-cols-[minmax(150px,1fr)_110px_70px] items-center gap-3 px-3 py-2"
              >
                ...
                <input
                  ...
                  onChange={(event) =>
                    setValues((current) => ({
                      ...current,
                      [student.student_id]: { ...value, marks: event.target.value },
                    }))
                  }
                ...
                <input
                  ...
                  onChange={(event) =>
                    setValues((current) => ({
                      ...current,
                      [student.student_id]: { marks: '', absent: event.target.checked },
                    }))
                  }
                  className="h-4 w-4"
                />
          ...
          {!students.length && (
            <p className="text-muted-foreground p-6 text-center text-sm">No enrolled students for this schedule.</p>
          )}
        </div>
      </div>
