      ...
      <DialogContent className="glass rounded-2xl border border-border/60 backdrop-blur-xl sm:max-w-lg">
        ...
        <form ref={formRef} action={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            />
            <p className="text-xs text-muted-foreground">{LECTURE_VIDEO_URL_HINT}</p>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label htmlFor="lecture-course">Course</Label>
              <Input id="lecture-course" name="courseName" defaultValue={lecture?.course_name ?? ""} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="lecture-semester">Semester</Label>
              <Input id="lecture-semester" name="semester" defaultValue={lecture?.semester ?? ""} />
            </div>
          </div>
