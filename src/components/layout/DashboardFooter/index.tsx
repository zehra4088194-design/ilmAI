...
export function DashboardFooter() {
  ...
  return (
    <footer className="border-border/60 text-muted-foreground flex flex-wrap items-center justify-center gap-x-3 gap-y-1 border-t px-3 py-3 pb-20 text-center text-[11px] sm:px-4 sm:pb-3">
      ...
      {SOCIAL_LINKS.map((link) => <a key={link.href} href={link.href} target="_blank" rel="noopener noreferrer" className="hover:text-foreground hover:underline">{link.label}</a>)}
      {WHATSAPP_CONTACT_NUMBER && <a href={`https://wa.me/${WHATSAPP_CONTACT_NUMBER}`} target="_blank" rel="noopener noreferrer" className="hover:text-foreground inline-flex items-center gap-1 hover:underline"><MessageCircle className="h-3 w-3" />WhatsApp: +{WHATSAPP_CONTACT_NUMBER}</a>}
    </footer>
  );
}
