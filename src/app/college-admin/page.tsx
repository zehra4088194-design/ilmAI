...
export default async function CollegeAdminHomePage() {
  ...
  const [pending, lectures, resources, students] = await Promise.all([
    ...
    getCollegeResources(supabase, context.college.id),
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {cards.map((card) => (
          <Link
            key={card.href}
            href={card.href}
            className="glass flex flex-col gap-2 rounded-2xl border border-border/60 bg-card/60 p-5 backdrop-blur-xl transition-shadow hover:shadow-md"
          >
