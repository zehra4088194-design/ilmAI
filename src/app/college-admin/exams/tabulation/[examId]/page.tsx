...
import { getCollegeExamTabulation } from '@/lib/college-erp/queries';
import { ResultTabulationTable } from '@/components/features/exams/ResultTabulationTable';
import { cn } from '@/lib/utils/cn';

export default async function CollegeExamTabulationPage({
  params,
  searchParams,
}: {
  ...
  const data = await getCollegeExamTabulation(supabase, context, examId);
  if (!data) notFound();

  const activeView = view === 'merit' ? 'merit' : 'sheet';
  const rows = section ? data.rows.filter((row) => row.sectionLabel === section) : data.rows;
  const baseHref = `/college-admin/exams/tabulation/${examId}`;
