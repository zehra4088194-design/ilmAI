...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await (supabase.from('student_application_templates') as any)
...
export async function DELETE(req: NextRequest) {
  ...
  const { error } = await (supabase.from('student_application_templates') as any).delete().eq('id', id).eq('student_id', user.id);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
