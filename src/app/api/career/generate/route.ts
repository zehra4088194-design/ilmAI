...
export async function POST(req: NextRequest) {
  try {
    ...
    const [{ data: profile }, { data: inputs }, { data: twin }, { data: cached }] = await Promise.all([
      supabase
        .from('profiles')
        .select('subscription_tier, board, grade_level, target_marks_percentage, total_marks_percentage')
        .eq('id', user.id)
        .single(),
      supabase
        .from('career_profile_inputs' as any)
        .select('*')
    ...
    if (!limit.success)
      return NextResponse.json(
        { status: 'error', error: await getConfiguredLimitExceededMessage(tier, 'Career AI') },
        { status: 429 }
      );
    ...
    if (
      !refresh &&
      cacheRow &&
      (!inputsRow?.updated_at || new Date(cacheRow.generated_at) >= new Date(inputsRow.updated_at))
    ) {
      ...
      recommended_careers: [
        {
          title: 'Data Analyst',
          match_score: 72,
          description: 'Study pathway suggestion based on selected interests and subjects.',
          avg_salary_pkr: 'varies by city and experience',
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are an evidence-conscious career counselor for Pakistani students. Do not guarantee admissions, salaries, visas, or job outcomes. Use estimates/ranges and flag low-confidence claims. Return only JSON.',
        },
    ...
    await consumeAiCredits(user.id, tier, 'career');
