...
function normalizeLanguage(value: unknown): (typeof LANGUAGES)[number] {
  return typeof value === 'string' && LANGUAGES.includes(value as (typeof LANGUAGES)[number]) ? value as (typeof LANGUAGES)[number] : 'en';
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await gatewayChat({
      ...
      tier: 'mini',
      messages: [
        { role: 'system', content: 'Create one short speaking practice prompt for a student. Return only plain text, no markdown.' },
        { role: 'user', content: `Language: ${language}. Difficulty: ${difficulty}. Make it useful for pronunciation and viva confidence.` },
      ],
      maxTokens: 120,
      temperature: 0.7,
    });
    ...
    return NextResponse.json({ status: 'success', data: { prompt: 'Explain your favorite topic from today in three clear sentences.' } });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!limit.success) {
      return NextResponse.json({ status: 'error', error: await getConfiguredLimitExceededMessage(tier, 'Speaking Practice') }, { status: 429 });
    }
    ...
    const result = await gatewayChat({
      ...
      messages: [
        { role: 'system', content: 'You are a pronunciation coach. Return JSON with keys score (0-100), feedback, follow_up_question, quick_revision_notes.' },
    ...
    const parsed = parseAiJson<{ score: number; feedback: string; follow_up_question?: string; quick_revision_notes?: string }>(result.text, {
      score: 70,
    ...
    const { data } = await db.from('speaking_practice_sessions').insert({
      student_id: user.id,
      language,
