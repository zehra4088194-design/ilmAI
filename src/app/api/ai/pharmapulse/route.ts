...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'suggestions') {
      ...
      const suggestionProvider = await resolveAiRoutingProvider('studyTools');
    ...
    const pharmaProvider = await resolveAiRoutingProvider('studyTools');
    const result = await gatewayChat({
      provider: pharmaProvider,
      strictProvider: true,
      routingPolicy: 'text',
      tier: 'pro',
      messages: [
