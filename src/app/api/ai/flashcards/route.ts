  ...
  try {
    ...
    if (!limit.success)
      return NextResponse.json(
        { status: 'error', error: await getConfiguredLimitExceededMessage(tier, 'Flashcards AI') },
        { status: 429 }
      );
    ...
    // read them back via .select() — don't assign our own ids here (nanoid()
    ...
    await consumeAiCredits(user.id, tier, 'flashcards');
