  ...
  try {
    ...
    if (!limitCheck.success) {
      return NextResponse.json(
        { status: 'error', error: 'The daily AI limit has been reached. Try again tomorrow or upgrade to Pro.' },
        { status: 429 }
      );
    }
...
Include: 8 MCQs, 5 short questions, 3 long questions. Mark each as high or medium likelihood.`;
    ...
    const result = await gatewayChat({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert Pakistani board exam question predictor. Return only valid JSON.',
        },
    ...
    return NextResponse.json(
      { status: 'error', error: 'The guess paper could not be generated. Please try again.' },
      { status: 500 }
    );
  }
}
