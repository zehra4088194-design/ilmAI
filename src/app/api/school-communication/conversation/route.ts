...
export async function POST(req: NextRequest) {
  ...
  if (!otherProfileId || otherProfileId === user.id) {
    return NextResponse.json({ error: 'Choose another school member.' }, { status: 400 });
  }
  ...
  if (!isAllowedRolePair(callerRole, target.member_role)) {
    return NextResponse.json({ error: 'This school communication is not allowed for your role.' }, { status: 403 });
  }
  ...
  if ((callerRole === 'parent' && target.member_role === 'student') || (callerRole === 'student' && target.member_role === 'parent')) {
    ...
    if (!(await areParentStudentLinked(db, context.organization.id, parentId, studentId))) {
      return NextResponse.json({ error: 'Parent and student chats are only available for linked family members.' }, { status: 403 });
    }
  }

  const admin = await createAdminClient();
  const first = user.id < otherProfileId ? user.id : otherProfileId;
  const second = user.id < otherProfileId ? otherProfileId : user.id;
  const adminDb = admin as any;

  const { data: existing } = await adminDb
    .from('direct_conversations')
    .select('id, context_type, organization_id, relationship_type, participant_one_id, participant_two_id, last_message_at, created_at')
    .eq('context_type', 'school')
    .eq('organization_id', context.organization.id)
    .eq('relationship_type', 'school_member')
    .eq('participant_one_id', first)
    .eq('participant_two_id', second)
    .maybeSingle();
  ...
  const { data: conversation, error } = await adminDb
    ...
    .insert({
      ...
      relationship_type: 'school_member',
