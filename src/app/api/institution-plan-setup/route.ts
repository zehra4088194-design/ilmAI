...
export async function POST(request: Request) {
  ...
  const { data: membership } = await (supabase as any).from(membershipTable).select('member_role').eq('organization_id', organizationId).eq('profile_id', user.id).eq('status', 'active').maybeSingle();
  if (!membership || !['owner','admin'].includes(membership.member_role)) return NextResponse.json({ error: 'Only the institution owner/admin can change this plan.' }, { status: 403 });

  const admin = (await createAdminClient()) as any;
  const { data: cfg } = await admin.from('institution_ai_plan_config').select('free_student_allowance,student_price_usd').eq('institution_type', institutionType).eq('plan_code', planCode).eq('is_active', true).maybeSingle();
  ...
  if (scope === 'institution_wide' && planCode !== 'CUSTOM') update.grant_tier = planCode.toLowerCase();
  const { error } = await admin.from(settingsTable).update(update).eq('organization_id', organizationId);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });

  if (institutionType === 'school') await syncOrganizationSchoolGrants(organizationId, scope === 'institution_wide');
  else await syncOrganizationCollegeGrants(organizationId, scope === 'institution_wide');
