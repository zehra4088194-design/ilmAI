...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!title?.trim() || !body?.trim())
      return NextResponse.json({ status: 'error', error: 'Enter both a title and a question.' }, { status: 400 });
    ...
    try {
      ...
      const aiReply = await gatewayChat({
        ...
        messages: [
          {
            role: 'system',
            content: `You are an expert Pakistani board exam teacher (Class 9-12). Answer student questions clearly, patiently, and encouragingly in professional English. Use Roman Urdu only when the student explicitly requests it. Be thorough but concise.\n\n${MARKDOWN_ANSWER_FORMAT_INSTRUCTION}`,
          },
      ...
      const { data: aiTeacher } = await supabase
        .from('profiles')
        .select('id')
      ...
      if (aiTeacher) {
        ...
        if (!replyError) await consumeAiCredits(user.id, tier, 'doubt_teacher');
      }
    } catch (aiErr) {
...
export async function GET(req: NextRequest) {
  try {
    ...
    let query = supabase
      .from('doubts')
      .select(
        '*, profiles(full_name, avatar_url), doubt_replies(id, body, is_accepted, created_at, profiles(full_name, is_ai_operated))'
      )
