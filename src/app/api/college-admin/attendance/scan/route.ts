...
// Mirrors src/app/api/school-admin/attendance/scan/route.ts exactly, pointed at college tables.
...
export async function POST(req: NextRequest) {
  try {
    const { supabase, user, context } = await requireCollegeContext('attendance.manage', 'attendance');
    ...
    const extraction = await gatewayChat({
      ...
      messages: [
        {
          role: 'system',
          content: `You read OCR text of a handwritten college attendance register and extract each student row.
Rules:
...
- Do not invent students not present in the text. Do not skip any row you can read.`,
    ...
    const { data: enrollments } = await client
      ...
      .select('student_id, roll_number, profiles!college_enrollments_student_id_fkey(full_name)')
      .eq('organization_id', context.organization.id)
