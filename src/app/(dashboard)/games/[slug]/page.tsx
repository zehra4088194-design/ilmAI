...
import { GameRoomClient } from '@/components/features/games/GameRoomClient';
...
export default async function GamePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: row } = await (supabase.from('games' as any) as any)
    .select('id, slug, title, description, thumbnail_url, category, game_type, difficulty, featured, min_tier')
    .eq('slug', slug)
    .maybeSingle();
  const game = (row || DEFAULT_GAMES.find((item) => item.slug === slug)) as GameCardData | undefined;
  if (!game) notFound();
  ...
  return (
    <GameRoomClient
      game={game}
      tier={tier}
      canPlay={plan.access.games && limitSeconds > usedSeconds}
      initialRemainingSeconds={Math.max(0, limitSeconds - usedSeconds)}
    />
  );
}
