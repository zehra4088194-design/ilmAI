  ...
  return (
    <div className="space-y-6">
      <SchoolPageHeader
        ...
        description="Campuses, sessions, classes, sections, and subject ownership."
      />
      {['owner', 'admin'].includes(context.membership.member_role) && (
        <Card className="border-emerald-500/25 bg-emerald-500/5">
          <CardHeader>
            <CardTitle className="text-base">Principal control center</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <Link
              href="/school-admin/reports"
              className="border-border bg-background hover:bg-accent rounded-lg border p-3 transition-colors"
            >
              <span className="block text-sm font-semibold">Reports</span>
              <span className="text-muted-foreground mt-1 block text-xs">
                Review attendance, fees, and academic performance.
              </span>
            </Link>
            <Link
            ...
            >
              ...
              <span className="text-muted-foreground mt-1 block text-xs">
                Manage staff, teachers, students, and access.
              </span>
            </Link>
            <Link
              ...
              className="border-border bg-background hover:bg-accent rounded-lg border p-3 transition-colors"
            >
              <span className="block text-sm font-semibold">Communication</span>
              <span className="text-muted-foreground mt-1 block text-xs">Stay connected with families and staff.</span>
            </Link>
            <Link
              href="/settings"
              className="border-border bg-background hover:bg-accent rounded-lg border p-3 transition-colors"
            >
              <span className="block text-sm font-semibold">My account</span>
              <span className="text-muted-foreground mt-1 block text-xs">
      ...
      <Card>
        ...
        <CardContent>
          <SchoolActionForm
            action={updateSchoolOrganization}
          ...
          >
            ...
            <Input
              name="currency"
              defaultValue={context.organization.currency}
              maxLength={3}
              placeholder="PKR"
              required
            />
            <Input
              name="email"
              type="email"
              defaultValue={context.organization.email || ''}
        ...
        <CardContent className="flex flex-col gap-4 sm:flex-row sm:items-start">
          ...
          <div className="flex-1">
            <p className="text-muted-foreground mb-2 text-xs">
              Every enrolled member sees this logo (plus &quot;{context.organization.name} · Ilm AI&quot;) instead of
              the default Ilm AI mark on their dashboard.
            </p>
            <SchoolActionForm
        ...
        <CardContent className="flex flex-col gap-4 sm:flex-row sm:items-start">
          {context.organization.principal_signature_url && (
            ...
            <img
              ...
              className="border-border h-16 w-32 shrink-0 rounded-lg border bg-white object-contain p-1"
            />
          )}
          <div className="flex-1">
            <p className="text-muted-foreground mb-2 text-xs">
            ...
            <SchoolActionForm
              action={updateSchoolPrincipalSignature}
              submitLabel="Save"
            ...
            >
              <Input
                ...
                defaultValue={context.organization.principal_name || ''}
              />
      ...
      <div className="grid gap-5 lg:grid-cols-2">
          ...
          <CardContent>
            <SchoolActionForm action={createCampus} submitLabel="Add campus">
              ...
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" name="is_main" /> Main campus
              </label>
            </SchoolActionForm>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Add academic year</CardTitle>
          </CardHeader>
          <CardContent>
            <SchoolActionForm action={createAcademicYear} submitLabel="Add academic year">
              ...
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" name="is_current" /> Set as current
              </label>
            </SchoolActionForm>
          </CardContent>
        </Card>
      ...
      {canManageAcademics && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Subject offering</CardTitle>
          </CardHeader>
          <CardContent>
            <SchoolActionForm
              ...
              {data.sections.map((section: any) => {
                ...
                return (
                  <tr key={section.id} className="border-b last:border-0">
                    <td className="py-2">{classItem?.school_campuses?.name || '-'}</td>
                    <td>{classItem?.school_academic_years?.name || '-'}</td>
