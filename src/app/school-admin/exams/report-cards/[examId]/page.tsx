...
import { getExamReportCards } from '@/lib/school-erp/queries';
...
export default async function ExamReportCardsPage({
  params,
  searchParams,
}: {
  ...
  const result = await getExamReportCards(supabase, context, examId);
  if (!result) notFound();
    ...
    <div className="space-y-6">
      ...
      <div className="print:hidden">
        ...
        <p className="text-muted-foreground text-sm">
          {cardData.length} published report card{cardData.length === 1 ? '' : 's'}. Pick a design, then print or save as
          PDF — every card below renders in the chosen template.
        </p>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {REPORT_CARD_TEMPLATES.map((option) => (
            <Link
              key={option.key}
              href={`/school-admin/exams/report-cards/${examId}?template=${option.key}`}
              className={`rounded-xl border p-4 text-left transition ${
                option.key === template.key ? 'border-violet-500 bg-violet-500/10' : 'border-border hover:bg-muted'
              }`}
            >
      ...
      {cardData.length === 0 ? (
        <p className="text-muted-foreground print:hidden">
          No published report cards for this exam yet — publish results from the Exams page first.
        </p>
      ) : (
        <div className="space-y-8">
          {cardData.map((data, index) => (
            <Component key={`${data.studentName}-${index}`} data={data} />
          ))}
        </div>
      )}
    </div>
