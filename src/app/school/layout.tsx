...
import { LayoutDashboard, ClipboardList, CalendarClock, FileText, Bell, Library, Users, Sparkles, Search, Award, Presentation } from 'lucide-react';

const items = [
  ['Dashboard', '/school', LayoutDashboard],
  ['My Institution', '/school/student-hub', Users],
  ['Attendance', '/school#attendance', ClipboardList],
  ['Timetable', '/school#timetable', CalendarClock],
  ['Assignments', '/school/todays-homework', FileText],
  ['Exams & Results', '/school#results', FileText],
  ['Notices', '/school#notices', Bell],
  ['Institution Library', '/school/library', Library],
  ['Institution AI', '/school/institution-ai', Sparkles],
  ['Certificates & Records', '/school/documents', Award],
  ['Search anything', '/search', Search],
  ['Teacher AI Tools', '/school/teacher-tools', Presentation],
] as const;
  ...
  return <div>
    <div className="border-border bg-card/70 sticky top-0 z-20 border-b backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl gap-1 overflow-x-auto px-4 py-2 sm:px-6">
        ...
        {teacherNav && <Link href="/school/teacher-tools" className="flex shrink-0 items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-semibold text-muted-foreground transition hover:bg-muted hover:text-foreground"><Presentation className="h-3.5 w-3.5" />Teacher AI Tools</Link>}
      </div>
    </div>
    {children}
  </div>;
