...
export default async function SchoolStudentHubPage() {
  ...
  if (context.membership.member_role === 'parent') {
    const { data: guardians } = await db.from('school_guardians').select('student_id').eq('organization_id', context.organization.id).eq('guardian_id', user.id);
  ...
  const [{ data: profiles }, { data: enrollments }, { data: attendance }, { data: reportCards }, { data: invoices }, { data: timetable }, { data: assignments }, { data: announcements }, { data: events }, { data: notifications }] = await Promise.all([
    ...
    db.from('school_enrollments').select('student_id,roll_number,section_id,school_sections!school_enrollments_section_id_fkey(name,school_classes!school_sections_class_id_fkey(name))').eq('organization_id',context.organization.id).in('student_id',safeIds).eq('status','active').order('created_at',{ascending:false}),
    db.from('school_attendance_records').select('id,student_id,attendance_date,status').eq('organization_id',context.organization.id).in('student_id',safeIds).order('attendance_date',{ascending:false}).limit(120),
    db.from('school_report_cards').select('id,student_id,percentage,gpa,grade,class_position,summary,published_at,school_exams(name)').eq('organization_id',context.organization.id).in('student_id',safeIds).not('published_at','is',null).order('published_at',{ascending:false}).limit(12),
    db.from('school_fee_invoices').select('id,student_id,voucher_number,total_amount,paid_amount,due_date,status').eq('organization_id',context.organization.id).in('student_id',safeIds).order('due_date',{ascending:false}).limit(20),
    db.from('school_timetable_entries').select('id,section_id,subject_name,starts_at,ends_at,room,day_of_week').eq('organization_id',context.organization.id).order('day_of_week').order('starts_at'),
    db.from('school_homework').select('id,section_id,title,instructions,due_at,created_at').eq('organization_id',context.organization.id).order('due_at',{ascending:true}).limit(30),
    db.from('school_announcements').select('id,title,body,priority,published_at').eq('organization_id',context.organization.id).not('published_at','is',null).order('published_at',{ascending:false}).limit(10),
    db.from('school_calendar_events').select('id,title,event_type,starts_at').eq('organization_id',context.organization.id).gte('starts_at',new Date().toISOString()).order('starts_at').limit(10),
