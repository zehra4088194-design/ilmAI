...
export default async function SchoolReportCardPage({
  ...
  const { supabase, user, context } = await requireSchoolContext('exams.read');
  if (!user) redirect(`/login?redirect=${encodeURIComponent(`/school/report-card/${id}`)}`);
  if (!context) redirect('/dashboard');

  const { data: report } = await (supabase as any)
    .from('school_report_cards')
    .select('*, school_exams(name, term, starts_on, ends_on), profiles!school_report_cards_student_id_fkey(full_name)')
    .eq('id', id)
  ...
  const exam = related(report.school_exams);
  ...
  return (
    <main className="bg-muted/20 min-h-screen p-4 print:bg-white print:p-0 sm:p-8">
      <div className="mx-auto mb-4 flex max-w-4xl items-center justify-between gap-3 print:hidden">
        <Button asChild variant="ghost"><Link href="/school"><ArrowLeft className="mr-2 h-4 w-4" />Portal</Link></Button>
        <PrintReportButton />
      </div>
