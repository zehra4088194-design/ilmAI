...
export async function rescheduleMissedSessions(planId: string) {
  ...
  const remainingDays = Math.max(
    1,
    Math.ceil((new Date(`${examDate}T00:00:00`).getTime() - new Date(`${today}T00:00:00`).getTime()) / 86400000) + 1
  );
  const dailyLimit = Math.max(30, Math.round(Number(plan.daily_available_hours || 2) * 60));
  const compression =
    remainingDays < missed.length
      ? Math.max(15, Math.floor(dailyLimit / Math.ceil(missed.length / remainingDays)))
      : null;
