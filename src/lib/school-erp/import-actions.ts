...
// ============================================
// SCHOOL BULK IMPORT
// Onboarding a real school means 400 students and 25 staff. Typing them into
// one-record-at-a-time forms is days of work and was the single biggest
// blocker to onboarding an institution, so this imports them from the CSV the
// school already has.
//
// Students rarely have their own email in Pakistani schools, so when the CSV
// has no email we mint a deterministic login from the admission number and
// hand the school a credentials CSV to distribute.
// ============================================
...
import { INITIAL_SCHOOL_IMPORT_STATE, type SchoolImportState } from './import-types';
import { isOrganizationBillingActive, syncOrganizationSchoolGrants } from './subscription-cascade';
import type { SchoolContext } from './types';

// One import request stays inside a server action's time budget at this size.
// Bigger files are rejected with a clear instruction rather than half-imported.
const MAX_ROWS = 400;
const STAFF_ROLES = ['teacher', 'staff', 'accountant', 'admissions', 'admin'];

function fail(message: string): SchoolImportState {
  return { ...INITIAL_SCHOOL_IMPORT_STATE, message };
}
...
async function importContext(module: 'people') {
  ...
  if (!context || !hasSchoolPermission(context, 'people.manage')) {
...
function generatePassword() {
  // Ambiguous characters removed: these get read aloud and copied by hand.
...
function fallbackEmail(context: SchoolContext, identifier: string) {
...
/**
 * Finds an existing account by email, or creates one. Returns the profile id
 * plus the generated password when a new login was minted, so the caller can
 * give the school something to hand out.
 */
async function resolveProfile(
  ...
  if (error || !created?.user?.id) {
    // Losing a race with another import (or a parent signing up mid-run) is
    // not an error — reuse whatever account now exists.
...
async function upsertMembership(admin: any, context: SchoolContext, profileId: string, role: string, extra: any = {}) {
...
export async function importSchoolStudents(
  _state: SchoolImportState,
  formData: FormData
): Promise<SchoolImportState> {
  ...
  // Sections can be chosen once for the whole file, or named per row.
  ...
  for (const section of sections || []) {
    const className = (Array.isArray(section.school_classes) ? section.school_classes[0] : section.school_classes)?.name;
    sectionByName.set(String(section.name).toLowerCase(), section.id);
    if (className) sectionByName.set(`${className} ${section.name}`.toLowerCase(), section.id);
  }
  ...
  const state: SchoolImportState = { ...INITIAL_SCHOOL_IMPORT_STATE, errors: [], credentials: [] };
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    const admissionNumber = row.admission_number || row.admission_no || row.roll_number || '';
    ...
    try {
      ...
      const email = (row.email || '').trim().toLowerCase() || fallbackEmail(context, admissionNumber);
      ...
      const { error: enrollmentError } = await admin.from('school_enrollments').upsert(
        {
          organization_id: context.organization.id,
      ...
      if (guardianName || guardianEmail) {
        const resolvedGuardianEmail = guardianEmail || fallbackEmail(context, `g-${admissionNumber}`);
        ...
        await admin.from('school_guardians').upsert(
          {
            organization_id: context.organization.id,
            student_id: student.id,
      ...
      state.errors.push({
        row: line,
        name: fullName || admissionNumber || `Row ${line}`,
...
export async function importSchoolStaff(_state: SchoolImportState, formData: FormData): Promise<SchoolImportState> {
  let context: SchoolContext;
  let admin: any;
  try {
    ({ context, admin } = await importContext('people'));
...
async function finishImport(admin: any, context: SchoolContext, state: SchoolImportState, kind: string) {
  const touched = state.created + state.updated;
  if (touched && (await isOrganizationBillingActive(admin, context.organization.id))) {
    // Bulk grant so every freshly imported member gets the school's AI tier.
    await syncOrganizationSchoolGrants(context.organization.id, true);
  }
