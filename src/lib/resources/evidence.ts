...
export function buildResourceEvidence(title: string, context: string, confidence = 88): ResourceEvidence {
  ...
  const pageMatch =
    context.match(/(?:^|\n)\s*(?:page|p\.?)\s*[:#-]?\s*(\d{1,4})\b/i) || context.match(/\f\s*(?:page\s*)?(\d{1,4})?/i);
  return {
    title,
    excerpt: compact.slice(0, 420),
    confidence,
    pageReference: pageMatch?.[1]
      ? `Page ${pageMatch[1]}`
      : 'Extracted source context; exact page marker not available',
  };
}

export function buildResourceEvidenceFromChunk(
  title: string,
  chunk: { text: string; page_number?: number | null; metadata?: unknown },
  confidence = 92
): ResourceEvidence {
  ...
  return {
    ...
    excerpt: compact.slice(0, 420),
    confidence,
    pageReference: chunk.page_number
      ? `Page ${chunk.page_number}`
      : 'Stored source chunk; exact page marker not available',
  };
}
