...
export const publicCollegeAdmissionSchema = z.object({
  ...
  applyingForProgram: z.string().trim().min(1).max(80),
  guardianName: z.string().trim().min(2).max(120),
  guardianEmail: z.string().trim().email().max(254).optional().or(z.literal('')),
  // Phase 6e: the applicant's own email, if they already have an ilm AI account — lets the college
  // auto-enroll them later when this application is marked 'enrolled' (see updateCollegeAdmissionStatus).
  applicantEmail: z.string().trim().email().max(254).optional().or(z.literal('')),
  guardianPhone: z.string().trim().min(7).max(30),
  previousInstitution: z.string().trim().max(160).optional().or(z.literal('')),
