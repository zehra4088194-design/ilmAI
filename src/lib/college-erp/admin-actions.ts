...
// New-schema provisioning actions for /admin/colleges, mirroring
// src/app/(admin)/admin/schools/actions.ts exactly. Operates on college_organizations /
// college_campuses / college_memberships (the Phase 1+ ERP schema) — deliberately separate from
// the legacy colleges/college_admins actions in src/lib/college/*, which are untouched by this file.
...
export async function createCollegeOrganization(
  ...
  if (name.length < 2 || !slug || !ownerEmail.includes('@')) {
    return { success: false, message: 'College name, slug, and a registered owner email are required.' };
  }
  ...
  const { data: organization, error } = await db
    ...
    .insert({
      name,
      slug,
      organization_type: String(formData.get('organization_type') || 'college'),
      status: 'active',
  ...
  const { data: campus, error: campusError } = await db
    ...
    .single();
  if (campusError) {
    await db.from('college_organizations').delete().eq('id', organization.id);
    return { success: false, message: campusError.message };
  }

  const { error: membershipError } = await db.from('college_memberships').insert({
    organization_id: organization.id,
    campus_id: campus.id,
    profile_id: owner.id,
    member_role: 'owner',
  ...
  await db.from('college_audit_logs').insert({
    ...
    action: 'create',
    entity_type: 'college_organization',
...
export async function updateCollegePlanSettings(
  ...
  const enabledModules = normalizeCollegeModules(formData.getAll('enabled_modules')) || [];
  ...
  await db.from('college_audit_logs').insert({
    ...
    actor_user_id: admin.id,
    action: 'update',
    entity_type: 'college_plan_settings',
