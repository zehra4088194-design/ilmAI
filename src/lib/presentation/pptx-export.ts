  ...
  deck.slides.forEach((item, index) => {
    ...
    switch (item.type) {
      case 'title':
        ...
        slide.addText(item.title || deck.topic, {
          x: 0.9,
          y: 2.5,
          w: width - 1.8,
          h: 1.25,
        ...
        break;
      case 'two-column': {
        slide.addText(item.title || 'Comparison', {
          x: 0.65,
          y: 0.65,
          w: width - 1.3,
          h: 0.65,
        ...
        [
          ...
          slide.addShape(pres.ShapeType.roundRect, {
            ...
            w: columnWidth,
            h: 4.75,
            fill: { color: theme.card, transparency: 5 },
            line: { color: theme.card },
          });
          slide.addText(col?.heading || 'Key points', {
            x: x + 0.3,
            y: 1.95,
            w: columnWidth - 0.6,
            h: 0.45,
          ...
          slide.addText(
            (col?.bullets || []).map((text) => ({ text, options: { bullet: { indent: 14 }, breakLine: true } })),
            {
              x: x + 0.35,
              y: 2.6,
              w: columnWidth - 0.7,
      ...
      case 'quote':
        slide.addText(`"${item.quote || item.title || deck.topic}"`, {
          x: 1.15,
          y: 2.25,
          w: width - 2.3,
          h: 1.8,
        ...
        break;
      case 'stats': {
        slide.addText(item.title || 'Key insights', {
          x: 0.7,
          y: 0.7,
          w: width - 1.4,
          h: 0.65,
        ...
        const cardWidth = Math.min(
          2.7,
          (width - 1.4 - gap * Math.max(stats.length - 1, 0)) / Math.max(stats.length, 1)
        );
      ...
      case 'chart': {
        slide.addText(item.title || 'Key concepts', {
          x: 0.7,
          y: 0.7,
          w: width - 1.4,
          h: 0.65,
      ...
      case 'section-break':
        ...
        slide.addText(item.title || 'Next section', {
          x: 1,
          y: 3.15,
          w: width - 2,
          h: 1,
        ...
        break;
      case 'closing':
        slide.addText(item.title || 'Thank You', {
          x: 0.9,
          y: 2.8,
          w: width - 1.8,
          h: 0.95,
        ...
        break;
      default:
        slide.addText(item.title || 'Slide', {
          x: 0.7,
          y: 0.7,
          w: width - 1.4,
          h: 0.65,
        ...
        slide.addText(
          (item.bullets || []).map((text) => ({ text, options: { bullet: { indent: 18 }, breakLine: true } })),
          { x: 1, y: 1.8, w: width - 2, h: 4.8, fontSize: 17, color: theme.text, fit: 'shrink' }
        );
        break;
    }
