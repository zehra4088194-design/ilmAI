...
export async function getCollegeWithCounts(
  supabase: SupabaseClient,
  slug: string
): Promise<CollegeWithCounts | null> {
 ...
 *  - neither -> "none"
 */
export async function getStudentCollegeState(
  supabase: SupabaseClient,
  userId: string
): Promise<StudentCollegeState> {
  const db = supabase as any;
